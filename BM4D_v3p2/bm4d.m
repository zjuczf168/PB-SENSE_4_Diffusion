...
transform_2D_Wie_name      = 'dct';     % 2-D spatial transform (Wiener filtering) dct
transform_3rd_dim_Wie_name = 'dct';     % 1-D spatial transform dct
transform_4th_dim_Wie_name = 'haar';    % 1-D nonlocal transform


% Hard-thresholding (HT) parameters:
N1                   = 4;      % cube has size (N1 x N1 x N3)  4  must be 2^n ;default 4
N3                   = min(N1,size(z,3));   % cube has size (N1 x N1 x N3)
Nstep                = 3;      % sliding step to process every next reference cube
N2                   = 16;     % maximum number of similar cubes  16
Ns                   = 11;     % search neighborhood size NsxNsxNs
tau_match            = 0.1;    % threshold for the cube-distance (should be in [0,1])
lambda_thr4D         = 2.7;    % hard-thresholding parameter  2.7
thresholding         = 'hard'; % tresholding type  'hard' £¿£¿£¿£¿
                               %  'hard' --> hard thresholding
                               %  'soft' --> soft thresholding
sharpening           = 1.00;    % sharpening alpha parameter (1 -> no sharpening)% default 1 /1.05
